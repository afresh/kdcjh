...
namespace EWeb.Tests
{
    public abstract class EWebTestBase : AbpIntegratedTestBase<EWebTestModule>
    {
        ...
        protected void UsingDbContext(Action<EWebDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<EWebDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<EWebDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<EWebDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<EWebDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EWebDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<EWebDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EWebDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<EWebDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<EWebDbContext, Task<T>> func)
        {
            T result;
